...
def normalize_sent_packages(row, most_common_timedelta):
    ...
    """
    normalized_count = row["value"] / (
        row["day_difference"] * 24 * 60 / most_common_timedelta
    )
...
def find_most_common_timedelta(timeseries):
    ...
    timeseries["timedelta_mins"] = timeseries.apply(
        calculate_total_mins, col_name="timedelta", axis=1
    )
    most_common_timedelta = (
        timeseries.groupby("timedelta_mins")
        .count()
...
def extract_main_parameter(parameter_list):
    ...
    """
    dp = DataPool(to_replace={"parameter": "variable"})
    all_parameters = dp.parameter.all()
    main_parameters = all_parameters[
        all_parameters["description"].str.startswith("Main measurement parameter")
    ]
...
def downsampling_faulty_sensors(dataframe, frequency):
    ...
    helper_df["artificial_timestamp"] = pd.date_range(
        start=min, end=max, freq=frequency
    )
    df_merged = pd.merge_asof(
        left=helper_df,
        right=dataframe,
        left_on="artificial_timestamp",
        right_on="timestamp",
        direction="nearest",
    )
...
def plot_packages_received_histogram(source_name, start_date, save_directory=""):
    ...
    if len(main_parameters) < 1:
        warnings.warn(
            "could not find a main measurement parameter for this sensor: {}".format(
                source_name
            )
        )
        f = open("log_histogram.txt", "a")
        f.write(
            "could not find a main measurement parameter for this sensor: {}\n".format(
                source_name
            )
        )
        f.close()
        ...
        warnings.warn(
            "found multiple main measurement parameters ({}) for this sensor: {}. All but {} will be discarded!".format(
                main_parameters, source_name, main_parameters[0]
            )
        )
        f = open("log_histogram.txt", "a")
        f.write(
            "found multiple main measurement parameters ({}) for this sensor: {}. All but {} will be discarded!\n".format(
                main_parameters, source_name, main_parameters[0]
            )
        )
        f.close()
    ...
    parameter_data["timedelta"] = parameter_data["timestamp"] - parameter_data[
        "timestamp"
    ].shift(1)
    parameter_data["timedelta_mins"] = parameter_data.apply(
        calculate_total_mins, col_name="timedelta", axis=1
    )
    ...
    plt.figure(dpi=1000)
    counts, bins, bars = ax[1].hist(
        plot_data,
        bins=range(min(plot_data), 200, 1),
        align="left",
    )

    plt.figure(dpi=1000)

    counts, bins, bars = ax[0].hist(
        plot_data,
        bins=range(min(plot_data), 24, 1),
        align="left",
    )
    ...
    if save_directory != "":
        save_directory3 = os.path.join(
            save_directory,
            "timedelta_histograms_zoomed_extra",
            source_name.split("_")[0],
        )
...
def plot_timeseries(source_name, start_date, save_directory=""):
    ...
    for parameter_value in main_parameters:
        ...
        fig = px.scatter(
            ...
            x="timestamp",
            y="value",
            title="{}_{}".format(source_name, parameter_value),
        )
        ...
        fig.write_html(
            os.path.join(
                save_directory,
                "timeseries",
                "{}_{}.html".format(source_name, parameter_value),
            )
        )
...
def calculate_PSR(
    ...
    start_date,
    resolution="M",
    allow_higher_samplingrates=True,
    thresh_drop_values=-100000,
    end_date="",
):
    """
    ...
    if allow_higher_samplingrates == False:
        ...
        parameter_data = downsampling_faulty_sensors2(
            parameter_data, frequency=frequency
        )

    parameter_data = parameter_data.drop(
        parameter_data[parameter_data.value < thresh_drop_values].index
    )

    # add a column with the difference in days of current and previous date:
    parameter_data["day_difference"] = (
        (parameter_data["timestamp"].dt.floor(freq="D"))
        - (parameter_data["timestamp"].shift(1).dt.floor(freq="D"))
    ) / np.timedelta64(1, "D")

    # groupby and count
    grouped_data = (
        parameter_data.groupby(pd.Grouper(key="timestamp", axis=0, freq=resolution))
        .agg({"day_difference": "sum", "value": "count"})
        .reset_index()
    )

    # normalize the count
    grouped_data["normalized_count"] = grouped_data.apply(
        normalize_sent_packages, most_common_timedelta=most_common_timedelta, axis=1
    )
...
def plot_boxplot(
    ...
    )  # find number of sources in specific group
    ...
    box_plot = sns.boxplot(
        data=data,
        y="value",
        x=grouping_attribute,
        hue=coloring_attribute,
        ax=ax,
    ...
    for c in box_plot.get_children():
        if type(c) == matplotlib.patches.PathPatch:
            ...
            label_of_box = list(color_label_dict.keys())[
                list(color_label_dict.values()).index(c.get_facecolor())
            ]

            # add a text at the correct position
            box_plot.text(
                (
                    c.get_extents().transformed(ax.transData.inverted()).extents[0]
                    + c.get_extents().transformed(ax.transData.inverted()).extents[2]
                )
                / 2,
                c.get_extents().transformed(ax.transData.inverted()).extents[3] + 1,
                label_of_box,
                horizontalalignment="center",
                size=10 / len(color_label_dict) * 12,
    ...
    if img_name != "":
        ...
        fig.savefig(
            os.path.join(save_directory, "{}.png".format(img_name)),
            dpi=100,
            bbox_inches="tight",
        )
...
def plot_flattened_signal(
    data,
    group_att="source_name",
    value_column="value",
    valid_parameters="",
    valid_sources="",
    ...
    for source in list(plotdata[group_att].unique()):
        plotdata_filtered = plotdata[plotdata["source_name"] == source].sort_values(
            "timestamp"
        )

        # filtering the signal:
        plotdata_filtered["value_after_min_max"] = plotdata_filtered[
            value_column
        ].apply(
            lambda x: min_allowed_value
            if x < min_allowed_value
            else (max_allowed_value if x > max_allowed_value else x)
        )
        plotdata_filtered["value_after_median"] = (
            plotdata_filtered["value_after_min_max"]
            .rolling(window=median_win_size, center=True)
            .median()
        )
        plotdata_filtered["trend"] = (
            plotdata_filtered["value_after_median"]
            .rolling(window=mean_win_size, center=True)
            .mean()
        )

        if normalize == "mean":
            plotdata_filtered["trend"] = (
                plotdata_filtered["trend"] - plotdata_filtered["value"].mean()
            ) / plotdata_filtered["trend"].std()

        if normalize == "median":
            plotdata_filtered["trend"] = (
                plotdata_filtered["trend"] - plotdata_filtered["value"].median()
            ) / plotdata_filtered["trend"].std()

        lineplot = sns.lineplot(
            data=plotdata_filtered, x="timestamp", y="trend", ax=ax, label=source
        )
