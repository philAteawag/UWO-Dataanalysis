    ...
    if how == "1_Sensor_x_Frequencies":
        for source in tqdm.tqdm(list(dp.source.all()["name"])):
            ...
            if source != "bt_dl942_11_veloweg_sd":
                fig, ax = plt.subplots(
                    len(frequency), sharey=True, figsize=(15, len(frequency))
                )
                ...
                for freq in frequency:
                    grouped_data_x = helper_functions.calculate_PSR(
                        source_name=source,
                ...
                for i, freq in enumerate(frequency):
                    sns.heatmap(
                        data=grouped_data_long[i],
                        annot=freq_annot_dict[freq],
                        cmap="RdYlGn",
                        vmin=0,
                        vmax=1,
                        ax=ax[i],
                    ...
                    if i < len(frequency) - 1:
                        ...
                        ax[i].set_xticklabels(
                            [
                                pd.to_datetime(t.get_text()).strftime("%D")
                ...
                fig.savefig(
                    os.path.join(savepath, "{}.png".format(source)),
                    dpi=1000,
                    bbox_inches="tight",
                )
                plt.close("all")
        ...
        for group in sensor_groups:
            ...
            if not os.path.exists(savepath):
                os.makedirs(savepath)
            nb_sensors_within_group = helper_functions.find_nb_of_sensor_within_group(
                group
            )  # needed to know how big our plot should be
            ...
            for source in tqdm.tqdm(list(dp.source.all()["name"])):
                try:
                    if source.split("_")[0] == group:
                        PSR_data = helper_functions.calculate_PSR(
                            source_name=source,
                            start_date=start_date,
            ...
            heatmap_plot = sns.heatmap(
                ...
                vmax=1,
                ax=ax,
                annot=freq_annot_dict[frequency[0]],
            )
            ...
            # title of the plot
            fig.suptitle(
                "PSR of {} sensors: Resolution : {}".format(group, frequency[0]),
                fontsize="x-large",
            )
            fig.savefig(
                os.path.join(savepath, "{}.png".format("grouped_plot")),
                dpi=1000,
                bbox_inches="tight",
            )
            plt.close("all")
...
def main(how, frequency, save_directory, start_date="2016-01-01", end_date=""):
    if how in ["x_Sensors_1_Frequency", "1_Sensor_x_Frequencies"]:
        ...
        print("invalid value for how was set!")
...
def parse_arguments():
    ...
    parser.add_argument(
        "--how",
        type=str,
        default="x_Sensors_1_Frequency",
        choices=["x_Sensors_1_Frequency", "1_Sensor_x_Frequencies"],
        help="Chose wheter you want a PSR Plot that is calcualted for one frequency \n"
    ...
    parser.add_argument(
        ...
        default="Q",
        nargs="+",
        help="Define which Frequencies should be used for the PSR Plot\n"
    ...
    parser.add_argument(
        "--start_date", default="2016-01-01", help="first date to consider"
    )
...
if __name__ == "__main__":
    ...
    main(
        how=arguments.how,
        frequency=arguments.frequencies,
