...
class Check_Signal_Sensor():
    ...
    def __init__(self, source_to_check, config_file=r'C:\Users\steineph\DataAnalysis\UWO-Dataanalysis\config_concept.json'):
        ...
        self.signal_to_check_against_current= self.load_signal(self.source_to_check_against, start=self._date1weekago, end = self._current_date, parameter_name=self.parameter_name)
        self.signal_to_check_against_historic= self.load_signal(self.source_to_check_against, start=self._date4monthago, end = self._date1weekago, parameter_name=self.parameter_name)
        self.diff_historic_rel, self.diff_historic_abs = self.get_dif_between2signals(df1=self.signal_to_check_historic, df2=self.signal_to_check_against_historic  )
        self.diff_current_rel, self.diff_current_abs = self.get_dif_between2signals(df1=self.signal_to_check_current, df2=self.signal_to_check_against_current  )
    ...
    def load_signal(self, source,parameter_name, start='2022-01-01', end=''):
        ...
        if end!='':
            ...
            data_loaded = dp.signal.get(source_name=source, start=start, end=end, parameter_name=parameter_name)
        data_loaded = helper_functions.downsample_data(data_loaded, groupby_att='source')
    ...
    def get_dif_between2signals(self, df1, df2, timestamp_col1='timestamp', timestamp_col2='timestamp', value_col1='value', value_col2='value', standardize=True):
        ...
        dataToCompare=pd.merge(data1, data2, left_index=True, right_index=True, how='inner', suffixes=('_x', '_y'))
        if standardize==True:
            dataToCompare = (dataToCompare-dataToCompare.mean())/dataToCompare.std()
        dataToCompare = dataToCompare.rolling(window=24).mean()
        dataToCompare['abs_difference']=dataToCompare['value_x']-dataToCompare['value_y']
        dataToCompare['rel_difference']=dataToCompare['abs_difference']/dataToCompare['value_y']
